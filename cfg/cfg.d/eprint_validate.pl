...
#	if ( $eprint->is_set("carousel_featured") && !$eprint->carousel_image()) {
#		my $fieldname = $xml->create_element( "span", class=>"ep_problem_field:carousel" );
#		push @problems, $repository->html_phrase("validate:need_creators_or_editors", fieldname=>$fieldname );
#	}

###################################################
# Example: If we don't have creators (eg. for a book) then we
