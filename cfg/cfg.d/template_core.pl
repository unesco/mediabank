...
EOJ
       ...
       $repo->current_url( host => 0, path => 'static' ),
       $repo->current_url( host => 0, path => 'cgi' ),
    ...
    $style->appendChild( 
        $repo->xml->create_text_node((defined $repo->current_user ? '.ep_not_logged_in' : '.ep_logged_in') . ' { display: none }' ) 
    );
    ...
    $head->appendChild( 
        $repo->make_javascript( undef, src => $repo->current_url( path => "static", "javascript/auto-".EPrints->human_version.".js" ), ) 
    );
    
	$head->appendChild( $repo->xml->create_text_node( "\n    " ) );

	# We remove fix for IE 6. We do not need it.
	...
	# meta-generator
	$head->appendChild( 
        $repo->xml->create_element( "meta", name => "Generator", content => "EPrints ".EPrints->human_version, ) 
    );
	$head->appendChild( $repo->xml->create_text_node( "\n    " ) );

	# meta-content-type
	$head->appendChild( 
        $repo->xml->create_element( "meta", 'http-equiv' => "Content-Type", content => "text/html; charset=UTF-8", ) 
    );
	$head->appendChild( $repo->xml->create_text_node( "\n    " ) );

	# meta-content-language
	$head->appendChild( 
        $repo->xml->create_element( "meta", 'http-equiv' => "Content-Language", content => $repo->get_langid ) 
    );
	$head->appendChild( $repo->xml->create_text_node( "\n    " ) );
