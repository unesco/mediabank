...
j(document).ready(function () {
    ...
    j(".ep_form_field_input").observe('childlist', ".epjs_ajax", function (record) {
 ...
 *
 */ 
    j('.search-panel .dropdown-menu').on('click', 'a.param', function(event) {
        event.preventDefault();
        var param = j(this).data("param");
        var concept = j(this).text();
        j('.search-panel span#search_concept').text(concept);
        j('.input-group #search_param_hidden').val(j('.input-group #search_param').val()+param);
        /* Act on the event */
    });
